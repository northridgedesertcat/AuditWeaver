...
echo [STEP 1/6] Starting Docker Compose services...
...
echo [STEP 2/6] Waiting for Elasticsearch to be ready...
...
echo [STEP 3/6] Waiting for Kafka to be ready...
call :WaitForKafka localhost:%KAFKA_PORT%
if errorlevel 1 (
    echo          WARNING: Kafka may not be fully ready, continuing anyway...
) else (
    echo          Kafka is ready!
    echo          Waiting extra 15 seconds for Kafka topics to initialize...
    timeout /t 15 /nobreak >nul
)

echo [STEP 4/6] Starting Rules Matching Engine...
...
echo [STEP 5/6] Starting Agent Module...
...
echo [STEP 6/6] Starting Django Backend and Next.js Frontend...
